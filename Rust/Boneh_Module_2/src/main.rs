...
fn main() {
    ...
    let mut cypherVec: Vec<u8> = decode_hex(&cypherText).unwrap();

    //cbc key
    let cbcKey = String::from("140b41b22a29beb4061bda66b6747e14");

    //ctr key
    let cbcKey = String::from("36f18357be4dbd77f050515c73fcf9f2");
    let mut cbcKeyVec: Vec<u8> = decode_hex(&cbcKey).unwrap();

    let (iv, data) = cypherVec.split_at_mut(16);

    println!("{:?} \n {:?}", iv, data);

    let gaKey: GenericArray<u8, U16> = GenericArray::clone_from_slice(cbcKeyVec.as_slice());

    println!("{:?}", gaKey);

    let cipher = Aes128::new(&gaKey);

    let test = &data[0..16];
    println!("{}",test.len());

    let block: GenericArray<u8, U16> = GenericArray::clone_from_slice(test);
    ...
    //let key_vec: Vec<u8> = decode_hex(key).unwrap();

    //let N : GenericArray::ArrayLength = 16;
    //let key_vec_gen_arr: GenericArray<u8, ArrayLength<U16> > = GenericArray::clone_from_slice(key_vec.as_slice());
    
    //let key_vec_gen_arr: GenericArray<u8, U16> = GenericArray::clone_from_slice(key_vec.as_slice());
    //println!("{:?}", key_vec_gen_arr);
